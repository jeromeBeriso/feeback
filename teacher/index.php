...
if (isset($_POST['login'])) {
    ...
    $teacher_exist = select(
        "SELECT * FROM `teacher` WHERE `ID_number` = ? LIMIT 1",
        [$data['ID']],
        "s"
    );

    if ($teacher_exist === false || mysqli_num_rows($teacher_exist) == 0) {
        alert('error','INVALID'); // ID not found
    } else {
        $teacher = mysqli_fetch_assoc($teacher_exist);
        ...
        if (!$teacher) {
            ...
            if (!password_verify($data['password'], $teacher['password'])) {
                ...
                $_SESSION['teacherFaculty'] = $teacher['faculty_id'];
                redirect('dashboard.php');
                echo 1; // success
            }
        }
    ...
    <div class="login-card shadow">
        ...
        <form method="POST">
            <div class="form-floating mb-3">
                <input type="text" class="form-control" id="floatingNumber" name="ID" placeholder="Password">
                <label for="floatingNumber">ID Number</label>
            </div>
            <div class="form-floating mb-3">
